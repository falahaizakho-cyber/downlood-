...
def start(message):
    ...
    if check_sub(user_id):
        bot.send_message(message.chat.id, f"سڵاو! لینکا ڤیدیۆیێ بفرێکه (TikTok, Insta, YT, FB).")
    else:
...
def handle_download(message):
    ...
    try:
        ...
        bot.edit_message_text(f"ئیشکال: فایلێ ڤێ لینکێ ناهێتە گرتن.", message.chat.id, msg.message_id)

# پێدڤییە بۆ Railway ئەڤ بەشە هەبیت
app = Flask('')
@app.route('/')
